...
export async function main() {
  const csvFilePath = "C:\\Users\\A\\Desktop\\sigam_vite\\BaseSicma_Urgence\\df_demInitial.csv";
  let totalRows = 0;
  let skippedRows = 0;
  const skippedIds: number[] = [];

  const rows: Prisma.demInitialCreateManyInput[] = [];
  ...
  fs.createReadStream(csvFilePath)
    ...
    .on('data', (row: any) => {
      ...
      rows.push({
        ...
        id_demInitial: parseInt(row.id_demInitial),
    ...
    .on('end', async () => {
      ...
      for (const row of rows) {
        try {
          await prisma.demInitial.create({ data: row });
        } catch (error: any) {
          skippedRows++;
          if (row.id_demInitial != null) {
            skippedIds.push(row.id_demInitial);
          }
          console.warn(`⚠️ Skipped demande id=${row.id_demInitial ?? 'unknown'} due to error: ${error.message}`);
        }
      }
