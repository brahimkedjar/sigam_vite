...
export async function main() {
  const csvFilePath = "C:\\Users\\A\\Desktop\\sigam_vite\\BaseSicma_Urgence\\df_demModification.csv";
  let totalRows = 0;
  let skippedRows = 0;
  const skippedIds: number[] = [];

  const rows: Prisma.demModificationCreateManyInput[] = [];
  ...
  fs.createReadStream(csvFilePath)
    ...
    .on('data', (row: any) => {
      ...
      rows.push({
        ...
        dossier_complet: parseBoolean(row.dossier_complet),
        rec_enquete_date: parseDate(row.rec_enquete_date) || null,
        rec_enquete_nomRespon: row.rec_enquete_nomRespon || null,
        obs_attestation: row.obs_attestation || null,
        ConResGeo: parseFloat(row.ConResGeo) || null,
        ConResExp: parseFloat(row.ConResExp) || null,
        VolumePrevu: parseFloat(row.VolumePrevu) || null,
        obs_sit_geo: row.obs_sit_geo || null,
        obs_empiet: row.obs_empiet || null,
        obs_emplacement: row.obs_emplacement || null,
        obs_geom: row.obs_geom || null,
        obs_superficie: row.obs_superficie || null,
        inscripProv: parseBoolean(row.inscripProv),
        intitule_projet: row.intitule_projet || null,
        type_modification: row.type_modification || null,
        montant_produit: parseFloat(row.montant_produit) || null,
      });
    })
    .on('end', async () => {
      ...
      for (const row of rows) {
        try {
          await prisma.demModification.create({ data: row });
        } catch (error: any) {
          skippedRows++;
          if (row.id_modification != null) {
            skippedIds.push(row.id_modification);
          }
          console.warn(`⚠️ Skipped demande id=${row.id_modification ?? 'unknown'} due to error: ${error.message}`);
        }
      }
