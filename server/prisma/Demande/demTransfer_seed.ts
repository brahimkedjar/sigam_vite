...
export async function main() {
  const csvFilePath = "C:\\Users\\A\\Desktop\\sigam_vite\\BaseSicma_Urgence\\df_demTransfert.csv";
  let totalRows = 0;
  let skippedRows = 0;
  const skippedIds: number[] = [];

  const rows: Prisma.demTransfertCreateManyInput[] = [];
  ...
  fs.createReadStream(csvFilePath)
    ...
    .on('end', async () => {
      ...
      for (const row of rows) {
        try {
          await prisma.demTransfert.create({ data: row });
        } catch (error: any) {
          skippedRows++;
          if (row.id_transfert != null) {
            skippedIds.push(row.id_transfert);
          }
          console.warn(`⚠️ Skipped demande id=${row.id_transfert ?? 'unknown'} due to error: ${error.message}`);
        }
      }
