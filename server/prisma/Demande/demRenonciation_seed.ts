...
export async function main() {
  ...
  const rows: Prisma.demRenonciationCreateManyInput[] = [];
  ...
  fs.createReadStream(csvFilePath)
    ...
    .on('data', (row: any) => {
      totalRows++;
      rows.push({
        id_renonciation: parseInt(row.id_renonciation, 10),
        id_demande: parseInt(row.id_demande, 10),
        RenonOct: parseBoolean(row.RenonOct),
      });
    })
    .on('end', async () => {
      ...
      for (const row of rows) {
        try {
          await prisma.demRenonciation.create({ data: row });
        } catch (error: any) {
          skippedRows++;
          if (row.id_renonciation != null) {
            skippedIds.push(row.id_renonciation);
          }
          console.warn(`⚠️ Skipped demande id=${row.id_renonciation ?? 'unknown'} due to error: ${error.message}`);
        }
      }
