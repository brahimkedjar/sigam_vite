...
interface ProcedureTimeline {
  procedure: {
    ...
    legalDurationDays?: number | null;
    totalActualPhaseDays?: number | null;
    totalDelayVsLegal?: number | null;
  };
              ...
              <div className={styles.procedureSummary}>
                ...
                <div className={styles.dates}>
                  ...
                  {timeline.procedure.totalDuration && (
                    <span>
                      <FiClock /> Durée réelle procédure:{' '}
                      {formatDurationToDHM(timeline.procedure.totalDuration)}
                    </span>
                  )}
                  {typeof timeline.procedure.legalDurationDays === 'number' && (
                    <span>
                      <FiClock /> Délai légal global:{' '}
                      {timeline.procedure.legalDurationDays} jrs
                    </span>
                  )}
                  {typeof timeline.procedure.totalActualPhaseDays === 'number' && (
                    <span>
                      <FiClock /> Somme durées phases:{' '}
                      {timeline.procedure.totalActualPhaseDays} jrs
                    </span>
                  )}
                  {typeof timeline.procedure.totalDelayVsLegal === 'number' &&
                    timeline.procedure.totalDelayVsLegal > 0 && (
                      <span className={styles.delay}>
                        Retard global vs délai légal:{' '}
                        {timeline.procedure.totalDelayVsLegal} jrs
                      </span>
                    )}
                </div>
              </div>
                ...
                <div className={styles.phasesGrid}>
                  {timeline.phases.map((phase) => (
                    <div key={phase.id} className={styles.phaseCard}>
                      ...
                      <p>Début: {formatDate(phase.startDate)}</p>
                      <p>Fin: {formatDate(phase.endDate)}</p>
                      <p>
                        Durée réelle:{' '}
                        {phase.actualDuration
                          ? formatHoursToDHM(phase.actualDuration)
                          : 'En cours'}
                      </p>
                      {typeof phase.expectedDays === 'number' && (
                        <p>Durée estimée (référence): {phase.expectedDays} jrs</p>
                      )}
                      {typeof phase.delayDays === 'number' && phase.delayDays > 0 && (
                        <p className={styles.delay}>
                          Retard phase: {phase.delayDays} jrs
                        </p>
                      )}
                    </div>
                  ))}
            ...
            <div className={styles.timeline}>
              {timeline.steps.map((step) => (
                <div key={step.id} className={styles.timelineItem}>
                  <div className={styles.timelineMarker}>{getStatusIcon(step.status)}</div>
                  <div className={styles.timelineContent}>
                    ...
                    <div className={styles.stepDetails}>
                      ...
                      <div className={styles.durationInfo}>
                        <span>Durée prévue: {formatPlannedDuration(step.plannedDuration)}</span>
              ...
              <div className={styles.demandsGrid}>
                {timeline.demands.map((demand) => (
                  <div key={demand.id} className={styles.demandCard}>
                    ...
                    <p>
                      Statut:{' '}
                      <span className={styles[demand.status.toLowerCase()]}>
                        {demand.status}
                      </span>
                    </p>
